...
`BOT_TOKEN`, `MONGODB_URI` (required), `QUOTE_API_URI` (required), `MONGO_MAX_POOL`, `STATS_FLUSH_MS`, `MINI_APP_SHORT_NAME`, `MINI_APP_URL`, `BOT_API_ROOT` (self-hosted Bot API server; cloud by default), `HEALTH_PORT`, `HEALTH_HOST` (unset = 0.0.0.0; set 127.0.0.1 unless an external prober needs /metrics), `BOT_CONCURRENCY`, `GRAMADS_TOKEN`, `ADMIN_ID`, `LOG_LEVEL`, `NODE_ENV`.
